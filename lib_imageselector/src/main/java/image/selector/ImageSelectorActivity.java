...
import image.StatusBarUtils;
import image.selector.crop.AvatorImageCropperActivity;
import image.selector.crop.CropOverlayView;
...
//		StatusBarUtils.StatusBarLightMode(this,true);
	...
	public void onImagesSelected(MultiImageSelectorFragment.ImageSelectSource source, final ArrayList<String> images) {
		...
		switch (selectorMode) {
			...
			case IMAGE_SELECTOR_MODE_SINGLE_CROP:
				...
				Intent intent = new Intent(ImageSelectorActivity.this, AvatorImageCropperActivity.class);
	...
	@Override
	public void onMediaSelected(int mediaType, ArrayList<String> pathList) {
		if (mediaType!=MediaStore.Files.FileColumns.MEDIA_TYPE_VIDEO&&IMAGE_SELECTOR_MODE_SINGLE_CROP==selectorMode){
			Log.e(TAG, "start crop image, image path is " + pathList.get(0));
			Intent intent = new Intent(ImageSelectorActivity.this, ImageCropperActivity.class);
			intent.putExtra(ImageCropperActivity.EXTRA_IMAGE_PATH, pathList.get(0));
			intent.putExtra(ImageCropperActivity.EXTRA_IMAGE_CROP_SHAPE, cropShape);
			intent.putExtra(ImageCropperActivity.EXTRA_IMAGE_CROP_WIDTH_HEIGHT_SCALE, cropWHScale);
			startActivityForResult(intent, REQUEST_CROPPER_IMAGE);
			registerImageCropReceiver();
			...
			final Intent resIntent = new Intent();
			resIntent.putExtra(EXTRA_RESULT_IMAGE_SELECTED_MODE, selectorMode);
			resIntent.putExtra(EXTRA_RESULT_IMAGE_SELECTED_SOURCE, currentSource = MultiImageSelectorFragment.ImageSelectSource.Album.getTypeId());
			resIntent.putStringArrayListExtra(EXTRA_RESULT_IMAGE_SELECTED_PATH, pathList);
			resIntent.putExtra(EXTRA_MEDIA_TYPE, mediaType);
			setResult(RESULT_OK, resIntent);
			resIntent.setAction(ACTION_IMAGE_SELECTOR);
			sendBroadcast(resIntent);
			finish();
